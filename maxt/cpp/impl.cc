...
export auto join(auto const& collection) {
...
using shared_random_data = std::shared_ptr<std::vector<char> const>;

export struct object_metadata {
  std::string bucket_name;
  std::string object_name;
  std::int64_t generation;
};

export struct iteration_config {
...
make_meter_provider(google::cloud::Project const& project,
                    std::string const& instance);

std::shared_ptr<opentelemetry::trace::TracerProvider>
make_tracer_provider(boost::program_options::variables_map const& vm) {
...
export auto make_prng_bits_generator() {
...
class usage {
 ...
 private:
  ...
  static auto as_nanoseconds(::timeval const& t) {
...
export void run(config cfg, named_experiments experiments) {
...
auto upload_objects(
    ...
    std::vector<std::string> const& object_names, shared_random_data data) {
  auto tracer = opentelemetry::trace::Provider::GetTracerProvider()->GetTracer(
      otel_sv(kAppName));
...
class sync_experiment : public experiment {
 public:
  ...
  std::vector<object_metadata> upload(std::mt19937_64& generator,
                                      config const& cfg,
                                      iteration_config const& iteration,
                                      shared_random_data data) override {
    auto tracer =
    ...
    for (auto& t : tasks) try {
        auto m = t.get();
        result.insert(result.end(), std::make_move_iterator(m.begin()),
...
export named_experiments make_experiments(
...
export void count_allocated_bytes(std::size_t count) {
